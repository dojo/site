...
The Dojo build produces separate platform bundles that will **only** be loaded based on two key conditions:

1) The shim module is imported somewhere in an application.
1) A user's browser does not natively support the browser feature.
...
This widget runs the `read-file.block.ts` module at build time, loading the file from the passed path and using its content as a child in the widget. The rendered application's HTML and CSS get written to the built `index.html`:
...
This addition of `before` and `after` middleware is a breaking change. However, if your application was already using `createCallbackDecorator` to create your middleware, then it should continue to work with version 5 of Dojo as the `createCallbackDecorator` function has been adapted to convert from the existing middleware API to the new API.

### Improvements for `serve` and `watch` options in `@dojo/cli-build-app`

The `@dojo/cli-build-app` command includes a number of improvements for running a development server:
